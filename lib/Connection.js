...
class Connection extends EventEmitter {
  ...
  query(query, params, callback) {
    ...
    function runQuery(callback) {
      let self = this;
      return self.connection.query(query, params, function(err, rows) {
        self._modifyRows(err, query, params, rows, 'query', callback);
      });
    }
  }
  ...
  selectOne(query, params, callback) {
    ...
    function runQuery(callback) {
      ...
      this.query(query, params, function (err, rows) {
        ...
        self._modifyRows(err, query, params, rows, 'selectOne', callback);
      });
    }

  }

  _modifyRows(err, query, params, rows, method, callback) {
    co(function *() {
      try {
        rows = yield sand.mysql.config.modifyRows(err, query, params, method, rows);
