  ...
  *load(row, opts) {
  ...
  static fromRow(rows, opts) {
    ...
    return co(function *() {
      ...
      if (_.isArray(rows)) {
        return pasync.mapLimit(rows, cls.mysql.config.mapLimit || MAP_LIMIT_DEFAULT, domain.bind(function (row) {
          return row instanceof cls ? Promise.resolve(row) : cls.create(row, opts);
        }));

      } else if (isNumericMap(rows)) {
        return pasync.mapValuesLimit(rows, cls.mysql.config.mapLimit || MAP_LIMIT_DEFAULT, domain.bind(function (row) {
          return row instanceof cls ? Promise.resolve(row) : cls.create(row, opts);
        }));
